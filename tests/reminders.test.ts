...
describe("Reminder tests", () => {
	...
	it("Should post reminders only on opened linked pull-requests", async () => {
		spyOn(
			await import("../src/helpers/task-metadata"),
			"getTaskAssignmentDetails",
		).mockReturnValue(Promise.resolve({ taskAssignees: [1] }));
		spyOn(
			await import("../src/helpers/task-metadata"),
			"parsePriorityLabel",
		).mockReturnValue(1);
		...
		).mockReturnValue(Promise.resolve({ id: 1 } as never));
		spyOn(
			await import("../src/helpers/get-assignee-activity"),
			"getAssigneesActivityForIssue",
		).mockReturnValue(Promise.resolve([]));
		spyOn(
			await import("../src/helpers/collect-linked-pulls"),
			"collectLinkedPullRequests",
		).mockReturnValue(
			Promise.resolve([
				{
					id: 2,
		...
		spyOn(
			await import("../src/helpers/structured-metadata"),
			"getCommentsFromMetadata",
		...
		await updateTaskReminder(
			...
			{
				number: 1,
				html_url: "https://github.com/ubiquity-os/daemon-disqualifier/issue/1",
