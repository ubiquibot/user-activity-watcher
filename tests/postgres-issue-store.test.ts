...
const trackedIssueUrl =
	"https://github.com/ubiquity-os-marketplace/daemon-disqualifier/issues/42";
...
describe("Postgres issue store", () => {
	...
	it("adds issues without duplicating rows", async () => {
		...
		try {
			...
			await expect(
				issueStore.getIssueNumbers(trackedIssueOwner, trackedIssueRepo),
			).resolves.toEqual([42]);
		} finally {
	...
	it("removes only the targeted issue number", async () => {
		...
		try {
			...
			await issueStore.removeIssueByNumber(
				trackedIssueOwner,
				trackedIssueRepo,
				41,
			);
	...
	it("groups repositories with stable issue ordering", async () => {
		...
		try {
			await issueStore.addIssue(
				"https://github.com/ubiquity-os-marketplace/daemon-disqualifier/issues/7",
			);
			await issueStore.addIssue(
				"https://github.com/ubiquity-os-marketplace/daemon-disqualifier/issues/3",
			);
			await issueStore.addIssue(
				"https://github.com/ubiquity-os/another-repo/issues/11",
			);
