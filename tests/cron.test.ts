...
describe("CRON tests", () => {
	...
	it("Should resolve action manifests from dist refs before the source ref", async () => {
		const { CronConfigurationHandler } = await import(
			"../src/cron/configuration"
		);
		const getManifest = spyOn(
			(await import("@ubiquity-os/plugin-sdk/configuration"))
		...
		getManifest.mockImplementation(async (plugin) => {
			...
			return plugin.ref === "dist/development"
				? (createMockManifest(
						`${plugin.owner}/${plugin.repo}@${plugin.ref}`,
					) as never)
				: null;
		});

		const handler = new CronConfigurationHandler(
			new Logs("debug"),
			{} as never,
		);
		...
		expect(manifest).toEqual(
			createMockManifest(
				"ubiquity-os-marketplace/daemon-disqualifier@dist/development",
			),
		);
	...
	it("Should fall back to the source ref when the dist manifest is missing", async () => {
		...
		expect(manifest).toEqual(
			createMockManifest(
				"ubiquity-os-marketplace/daemon-disqualifier@development",
			),
		);
		expect(getManifest.mock.calls).toEqual([
			[
	...
	it("Should run reminder sweep directly and clean stale issues", async () => {
		...
		const getIssue = mock(({ issue_number }: { issue_number: number }) => {
			if (issue_number === issue2.issueNumber) {
				return {
					data: { number: issue2.issueNumber, assignees: [], state: "closed" },
				};
			}
			return {
				data: {
		...
		spyOn(
			await import("@ubiquity-os/plugin-sdk/octokit"),
			"customOctokit",
		).mockReturnValue({
			rest: {
		...
		spyOn(
			...
			"populateDeadlineExtensionsThresholds",
		).mockImplementation(populateDeadlineExtensionsThresholds as never);
		spyOn(
			await import("../src/handlers/watch-user-activity"),
			"runRemindersForRepository",
		).mockImplementation(runRemindersForRepository as never);

		spyOn(
			await import("../src/adapters/postgres-issue-store"),
			"createPostgresIssueStore",
		).mockReturnValue(
			Promise.resolve({
				removeIssueByNumber,
				close,
		...
		expect(removeIssueByNumber).toHaveBeenCalledWith(
			owner,
			repo1,
			issue2.issueNumber,
		);
