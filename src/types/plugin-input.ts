...
export type ContextPlugin<TEvents extends SupportedEvents = SupportedEvents> =
	Context<PluginSettings, Env, null, TEvents> & {
		adapters: Adapters;
	};
...
export const pluginSettingsSchema = T.Object(
	{
		...
		negligenceThreshold: thresholdType({
			default: "7 days",
			description:
				"Delay to unassign users. 0 means disabled and any other value is counted in days, e.g. 7 days",
		...
		availableDeadlineExtensions: T.Object(
			{
				...
				amounts: T.Record(T.String(), T.Integer({ minimum: 1 }), {
					default: {},
				}),
			},
			{
				default: {},
				description:
					"The amount of deadline extensions available for a given priority label (how many reminders can be sent before disqualifying the user).",
				examples: [
					{
		...
		eventWhitelist: T.Transform(
			T.Array(T.String(), {
				...
				description:
					"List of webhook event names to consider as valid activity on a task",
				examples: [
					"pull_request.review_requested",
					"issue_comment.created",
					"push",
