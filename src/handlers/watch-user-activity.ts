...
export async function watchUserActivity(context: ContextPlugin) {
	...
	if (
		...
		if (commentData) {
			await context.adapters.issueStore.addIssue(
	...
	return {
		message: logger.warn(`Unsupported event ${context.eventName}`).logMessage
			.raw,
	};
}

export async function runRemindersForRepository(
	context: ContextPlugin,
	repo: ContextPlugin["payload"]["repository"],
) {
	context.logger.info(
		`> Watching user activity for repo: ${repo.name} (${repo.html_url})`,
	);
...
async function updateReminders(
	...
	for (const issue of issues) {
		if (shouldIgnoreIssue(issue)) {
			logger.debug(
				`Skipping issue ${issue.html_url} due to the issue not meeting the right criteria.`,
				{
					draft: issue.draft,
		...
		if (issue.assignees?.length || issue.assignee) {
			...
			logger.debug(
				`Skipping issue ${issue.html_url} because no user is assigned.`,
			);
