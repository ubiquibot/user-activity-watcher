 ...
 */
export async function getAssigneesActivityForIssue(
	context: ContextPlugin,
	issue: ListIssueForRepo,
	assigneeIds: number[],
) {
	const gitHubUrl = parseIssueUrl(issue.html_url);
	const issueEvents: GitHubTimelineEvents[] = await context.octokit.paginate(
	...
	const linkedPullRequests = await collectLinkedPullRequests(
		context,
		gitHubUrl,
	);
	for (const linkedPullRequest of linkedPullRequests) {
		const { owner, repo, issue_number } = parseIssueUrl(
			linkedPullRequest.url || "",
		);
		const events: GitHubTimelineEvents[] = await context.octokit.paginate(
			context.octokit.rest.issues.listEventsForTimeline,
			{
				owner,
				repo,
...
function filterEvents(
	issueEvents: GitHubTimelineEvents[],
	assigneeIds: number[],
) {
	...
	for (const event of issueEvents) {
		...
		if ("actor" in event && event.actor) {
			...
			(event.event === "committed" || event.event === "commented") &&
			"author" in event
		) {
