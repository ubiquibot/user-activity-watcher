...
export function createStructuredMetadata(
	...
	const metadataSerializedHidden = [
		ubiquityMetadataHeader,
		jsonPretty,
		"-->",
	].join("\n");
	...
	if (logMessage?.type === "fatal") {
		...
		metadataSerialized = [
			metadataSerializedVisible,
			metadataSerializedHidden,
		].join("\n");
	} else {
...
export async function getCommentsFromMetadata(
	...
	const ubiquityMetadataHeaderPattern = new RegExp(
		`<!-- ${HEADER_NAME} - ${className} - \\S+ - [\\s\\S]*?-->`,
	);
		...
		(response) =>
			response.data.filter(
				(comment) =>
					...
					ubiquityMetadataHeaderPattern.test(comment.body),
			),
	);
}
