...
export async function unassignUserFromIssue(
...
export async function remindAssigneesForIssue(
	context: ContextPlugin,
	issue: ListIssueForRepo,
) {
	const { logger, config } = context;
	...
	const hasLinkedPr = !!(
		await collectLinkedPullRequests(context, issueItem)
	).filter((o) => o.state === "OPEN").length;
	...
	if (config.followUpInterval <= 0) {
		...
		(config.pullRequestRequired && !hasLinkedPr) ||
		remainingExtensions <= 0
	) {
		logger.debug(
			"No linked pull-request or no more remaining extensions, will attempt to un-assign the user.",
			{
		...
		logger.info(
			`Passed the reminder threshold on ${issue.html_url} sending a reminder.`,
		);
		await remindAssignees(context, issue);
	}
}

async function shouldDisplayRemainingExtensionsReminder(
	context: ContextPlugin,
	issueAndPrTargets: IssuePrTarget,
) {
	const { octokit, logger } = context;
	const issueNumber =
		issueAndPrTargets.pr?.prNumber ?? issueAndPrTargets.issueNumber;
	const owner =
		issueAndPrTargets.pr?.prOwner ?? context.payload.repository.owner?.login;
	const repo = issueAndPrTargets.pr?.prRepo ?? context.payload.repository.name;

	if (!owner) {
		logger.warn(
			"No owner was found in the payload, won't display remaining extensions value",
			{ issueAndPrTargets },
		);
		return false;
	}

	let userAssignmentEvent = await getMostRecentUserAssignmentEvent(
		context,
	...
	if (!userAssignmentEvent && issueAndPrTargets.issueNumber !== issueNumber) {
		logger.debug(
			`No user assignment event was found, retrying with issue number ${issueAndPrTargets.issueNumber}`,
			{
				issueUrl: `https://github.com/${owner}/${repo}/issues/${issueAndPrTargets.issueNumber}`,
				issueAndPrTargets,
			},
		);
		userAssignmentEvent = await getMostRecentUserAssignmentEvent(
			context,
			{ owner: { login: owner }, name: repo },
			issueAndPrTargets.issueNumber,
		);
	}

	if (!userAssignmentEvent) {
		logger.debug(
			"No user assignment event was found, won't display remaining extensions value",
		);
		return false;
	}
	...
	const lastRemainingExtensionsValue = (
		...
		if (
			...
			new Date(event.created_at).getTime() >=
				new Date(userAssignmentEvent.created_at).getTime() &&
			...
			if (
				!lastRemainingExtensionsValue ||
				value < lastRemainingExtensionsValue
			) {
				return value;
			} else {
	...
	logger.debug("Last remaining extensions value", {
		lastRemainingExtensionsValue,
		remainingExtensions: issueAndPrTargets.remainingExtensions,
	});
...
async function buildReminderMessage(
	context: ContextPlugin,
	args: { issue: ListIssueForRepo; extensions: Extensions } & IssuePrTarget,
) {
	...
	const currentExtensionsMilestone =
		args.extensions.extensionsLimit - args.remainingExtensions + 1;
	...
	if (shouldDisplayExtensions && args.extensions.assignmentDate) {
		message.push(
			`The new deadline is on <code>**${new Date(
				args.extensions.assignmentDate.getTime() +
					(currentExtensionsMilestone + 1) * args.extensions.extensionTimeLapse,
...
export async function remindAssignees(
	...
	if (!config.pullRequestRequired) {
		...
		const openedLinkedPullRequests = (
			await collectLinkedPullRequests(context, { repo, owner, issue_number })
		)
		...
		for (const pullRequest of openedLinkedPullRequests) {
			...
			try {
				...
				logger.warn(
					`Could not post to ${pullRequest.url} will post to the issue instead.`,
					{ e },
				);
...
export async function removeEntryFromDatabase(
	context: ContextPlugin,
	issue: ListIssueForRepo,
) {
...
export async function closeLinkedPullRequests(
	context: ContextPlugin,
	issue: ListIssueForRepo,
) {
	const { octokit, logger } = context;
