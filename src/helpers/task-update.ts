...
import {
	closeLinkedPullRequests,
	remindAssignees,
	remindAssigneesForIssue,
	unassignUserFromIssue,
} from "./remind-and-remove";
import { getCommentsFromMetadata } from "./structured-metadata";
import {
	getMostRecentUserAssignmentEvent,
	getTaskAssignmentDetails,
	parsePriorityLabel,
} from "./task-metadata";

function getMostRecentActivityDate(
	assignedEventDate: DateTime,
	activityEventDate?: DateTime,
): DateTime {
	return activityEventDate && activityEventDate > assignedEventDate
		? activityEventDate
		: assignedEventDate;
}
...
export async function updateTaskReminder(
	...
	const assignedEvent = await getMostRecentUserAssignmentEvent(
		context,
		repo,
		issue,
	);
	...
	if (!assignedEvent) {
		logger.warn(
			`Failed to update activity for ${issue.html_url}, there is no assigned event.`,
		);
		return;
	}

	const activityEvent = (
		await getAssigneesActivityForIssue(
			context,
			issue,
			handledMetadata.taskAssignees,
		)
	)
	...
	const activityDate = activityEvent?.created_at
		...
		: undefined;
	let mostRecentActivityDate = getMostRecentActivityDate(
		assignedDate,
		activityDate,
	);
	...
	const lastReminders: RestEndpointMethodTypes["issues"]["listComments"]["response"]["data"][] =
		await Promise.all(
			openedLinkedPullRequestUrls.map(async (url) => {
				...
				const comments = await getCommentsFromMetadata(
					context,
					issue_number,
					owner,
					repo,
				...
				return comments.filter(
					(o) => DateTime.fromISO(o.created_at) > mostRecentActivityDate,
				);
			}),
		);
	...
	logger.info(
		`Handling metadata and disqualification threshold for ${issue.html_url}`,
		{
			now: now.toLocaleString(DateTime.DATETIME_MED),
			assignedDate: DateTime.fromISO(assignedEvent.created_at).toLocaleString(
				DateTime.DATETIME_MED,
			),
			lastReminderComment: lastReminderComment
				? DateTime.fromISO(lastReminderComment.created_at).toLocaleString(
						DateTime.DATETIME_MED,
					)
				: "NULL",
			mostRecentActivityDate: mostRecentActivityDate.toLocaleString(
				DateTime.DATETIME_MED,
			),
		},
	);
	...
	if (
		...
		mostRecentActivityDate.plus({
			milliseconds: prioritySpeed
				? followUpInterval / priorityLevel
				: followUpInterval,
		}) <= now &&
		...
		logger.info(
			"Will remind assignees because linked pull-requests are approved but not merged.",
			{
	...
	if (lastReminderComment) {
		...
		if (
			mostRecentActivityDate.plus({
				milliseconds: prioritySpeed
					? disqualificationTimeDifference / priorityLevel
			...
			mostRecentActivityDate.plus({ milliseconds: followUpInterval }) <= now
		) {
			await remindAssigneesForIssue(context, issue);
		} else {
			logger.debug(
				`Reminder was sent for ${issue.html_url} already, not beyond disqualification deadline threshold yet.`,
				{
					issue: issue.html_url,
					now: now.toLocaleString(DateTime.DATETIME_MED),
					assignedDate: DateTime.fromISO(
		...
		if (
			...
			logger.info(
				"Will attempt to remind assignees, no initial reminder was sent yet.",
				{
					issue: issue.html_url,
					mostRecentActivityDate: mostRecentActivityDate.toLocaleString(
						DateTime.DATETIME_MED,
			...
			logger.debug(
				`Nothing to do for ${issue.html_url} still within due-time.`,
				{
					issue: issue.html_url,
					now: now.toLocaleString(DateTime.DATETIME_MED),
					assignedDate: DateTime.fromISO(
