...
export async function areLinkedPullRequestsApproved(
	...
	for (const pullRequest of pullRequestsFromAssignees) {
		...
		try {
			...
			logger.info(
				`Pull request ${pullRequest.url} review decision: ${data.repository.pullRequest?.reviewDecision}`,
			);
			...
			logger.error(
				`Could not get pull-request approval state ${pullRequest.url}.`,
				{ e },
			);
		}
	}
