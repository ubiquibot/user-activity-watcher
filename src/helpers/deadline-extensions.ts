...
export async function getRemainingAvailableExtensions(
	...
	extensionTimeLapse: number;
	assignmentDate?: Date;
}> {
	const defaultExtensions = {
		remainingExtensions: 0,
		extensionsLimit: 0,
		extensionTimeLapse: 0,
	};
	const { config, logger } = context;
	const { owner, repo } = parseIssueUrl(issue.html_url);

	if (
		!config.negligenceThreshold ||
		!config.availableDeadlineExtensions.enabled
	) {
		return {
			remainingExtensions: Infinity,
			extensionsLimit: Infinity,
			extensionTimeLapse: Infinity,
		};
	}
	...
	const priorityLabel = issue.labels?.find(
		(label) =>
			typeof label !== "string" &&
			label.name &&
			priorityList.includes(label.name),
	);
	...
	const extensionsLimit =
		config.availableDeadlineExtensions.amounts[priorityLabel.name];
	let extensionTimeLapse = Math.max(
		1,
		config.negligenceThreshold / parsePriorityLabel([priorityLabel]),
	);
	...
	const assignmentEvent = await getMostRecentUserAssignmentEvent(
		context,
		{ owner: { login: owner }, name: repo },
		issue,
	);
	const assignmentDate = assignmentEvent?.created_at
	...
	const remainingExtensions = Math.ceil(
		extensionsLimit -
			extensionsLimit * (diffTime / (extensionsLimit * extensionTimeLapse)),
	);
