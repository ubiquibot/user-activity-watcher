...
export async function getMostRecentUserAssignmentEvent(
	context: ContextPlugin,
	repo: Repo,
	issue: ListIssueForRepo | number,
) {
	...
	logger.info(
		`Trying to find assignment event for the issue ${repo.owner.login}/${repo.name}/${issueNumber}`,
		{
	...
	const latestUserAssignment = assignedEvents.find((o) =>
		"assigner" in o ? o.assigner?.type === "User" : o.actor?.type === "User",
	);
	const latestBotAssignment = assignedEvents.find((o) =>
		"assigner" in o ? o.assigner?.type === "Bot" : o.actor?.type === "Bot",
	);
	let mostRecentAssignmentEvent = latestUserAssignment || latestBotAssignment;

	if (
		latestUserAssignment &&
		latestBotAssignment &&
		DateTime.fromISO(latestUserAssignment.created_at) >
			DateTime.fromISO(latestBotAssignment.created_at)
	) {
...
export async function getTaskAssignmentDetails(
	...
	const metadata = {
		taskAssignees: issue.assignees
			? issue.assignees.map((o) => o.id)
			: issue.assignee
				? [issue.assignee.id]
				: [],
	};
	...
	if (durationInMs === 0) {
		...
		logger.warn(
			`Invalid disqualification threshold found on ${issue.html_url}`,
		);
		return false;
	}
...
export function parsePriceLabel(
	...
	const priceLabel = labels
		?.map((label) => (typeof label === "string" ? label : label.name || ""))
		.find((name) => name.toLowerCase().startsWith("price:"));
