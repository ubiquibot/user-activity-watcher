...
type RepoIssue = Awaited<
	ReturnType<ContextPlugin["octokit"]["rest"]["issues"]["get"]>
>["data"];
...
async function getInstallationOctokit(
	appOctokit: ContextPlugin["octokit"],
	owner: string,
	repo: string,
	appAuth: AppAuth,
) {
...
export async function runCronJob() {
	...
	try {
		...
		logger.error("Cannot run CRON job due to missing GitHub App credentials.", {
			err,
		});
		throw err;
	}
	...
	try {
		...
		for (const { owner, repo, issueNumbers } of repositories) {
			...
			try {
				...
				const repoOctokit = await getInstallationOctokit(
					appOctokit,
					owner,
					repo,
					appAuth,
				...
				for (const issueNumber of issueNumbers) {
					...
					try {
						...
						const issueResponse = await repoOctokit.rest.issues.get({
							owner,
							repo,
							issue_number: issueNumber,
						});
						const hasAssignees = !!(
							issueResponse.data.assignee ||
							issueResponse.data.assignees?.length
						);
						if (issueResponse.data.state !== "open" || !hasAssignees) {
							logger.debug(
								"Removing entry due to issue closed or no assignees",
								{
									owner,
									repo,
									issueNumber,
						...
						logger.ok(
							`Selected issue for CRON reminder sweep (stopping after first valid issue).`,
							{
				...
				if (!config) {
					logger.warn(
						"No plugin configuration found for repository; skipping CRON reminder sweep.",
						{ owner, repo },
					);
					continue;
				}
				...
				logger.ok(
					"Processed repository updates directly through CRON reminder sweep.",
					{
						owner,
						repo,
