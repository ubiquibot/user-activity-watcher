...
export class CronConfigurationHandler extends ConfigurationHandler {
	override async getManifest(plugin: ManifestPlugin) {
		if (
			!isGithubPlugin(plugin) ||
			!plugin.ref ||
			plugin.ref.startsWith("dist/")
		) {
			return super.getManifest(plugin);
		}

		const distManifest = await super.getManifest({
			...plugin,
			ref: getDistRef(plugin.ref),
		});
		if (distManifest) {
			return distManifest;
		}
...
export async function resolveCronRepoConfig(
	...
	try {
		...
		const parsedConfig = await handler.getSelfConfiguration(manifest, {
			owner,
			repo,
		});
		if (!parsedConfig) {
		...
		logger.error("Failed to resolve repository configuration.", {
			owner,
			repo,
			err,
		});
